...
- [使用したUnity機能・アセット](#unity-features)
...
| プログラム / サポート | 各々 | 各担当プログラム、動作テスト、README更新 |
...
| 10/28 | キャラクター関連修正 | 渡瀬 | プレイヤー動作修正、敵動作修正、強敵作成（仮 |
| 10/29 | アイテム関連実装、戦闘関連修正 | 渡瀬 | アイテム関連のコード実装、敵キャラの追加修正 |
| 10/30 | 演出、サウンド、敵修正 | 渡瀬 | SE実装、パーティクル実装、遠距離敵実装 |
...
| **担当箇所で頑張った点** | 小松 | ナビメッシュがしばらくきちんと生成できず処理が重かったのですが、データの持ち方を変更したり設定を調べて解決できました。|
...
- マップはCSVファイルによるパーツ配置とし、プレハブを制作。方向が必要なものは1方向を作り、残り3方向をバリアントで回転。
- コードでの方向回転を考慮もしましたが、CSV編集での把握がしにくいため、あらかじめ各方向のデータを用意することにしました。
